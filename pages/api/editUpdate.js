...
const handler = async (req, res) => {
    if (req.method === "POST") {
        try {
            ...
            if (!req.body.UpdateID || !req.body.UpdateTitle || !req.body.UpdateDescription ) {
                return res.status(400).json({ success: false, msg: "Missing required fields in the request body." });
            }

            const { UpdateID, UpdateDescription, UpdateTitle,UpdateLink } = req.body;

            const forbiddenSymbols = /[^a-zA-Z0-9\s\-()]/;

            // Find the card in the database based on the provided cardid
            const foundCard = await Update.findOne({ UpdateID: UpdateID });
            ...
            foundCard.UpdateTitle = UpdateTitle;
            foundCard.UpdateDescription = UpdateDescription;
            foundCard.UpdateLink = UpdateLink;
